...
const DEFAULT_CHALLENGES=[{id:1,taskTemplateId:1,target:5,bonus:2},{id:2,taskTemplateId:2,target:3,bonus:2},{id:3,taskTemplateId:7,target:2,bonus:1},{id:4,taskTemplateId:6,target:1,bonus:3}];
const todayKey=()=>new Date().toISOString().slice(0,10);
async function getUsers(){const {data,error}=await supabase.from('app_users').select('username,role,player_name,is_active').order('created_at',{ascending:true}); if(error) throw error; return data||[];}
function buildState(users){
 ...
 return {players,entries:[],taskTemplates:DEFAULT_TASKS,dailyChallenges:DEFAULT_CHALLENGES,challengeDate:todayKey(),challengeBonusClaims:{},nextPlayerId:players.length+1,nextTaskTemplateId:11,observerEnabled:true};
}
async function saveState(state){
 const {error}=await supabase.from('app_settings').upsert({setting_key:'game_state',setting_value:state},{onConflict:'setting_key'}); if(error) throw error;
}
async function getState(){
 const users=await getUsers();
 const {data,error}=await supabase.from('app_settings').select('setting_value').eq('setting_key','game_state').maybeSingle(); if(error) throw error;
 let state=data?.setting_value||null;
 if(!state){state=buildState(users); await saveState(state);}
 if(!state.challengeDate||state.challengeDate!==todayKey()){state.challengeDate=todayKey(); state.challengeBonusClaims={};}
 ...
 if(!('observerEnabled' in state)) state.observerEnabled=true;
 return {state,users};
}
function getSession(req){const c=parseCookies(req); return verifySession(c.rmaquest_session, process.env.SESSION_SECRET);}
