...
const rebuildSteps=state=>{state.players.forEach(p=>p.steps=0); state.entries.forEach(e=>{const p=state.players.find(x=>x.name===e.playerName); if(p) p.steps += Number(e.points||0);});};
const countProgress=(state,ch)=>{const task=state.taskTemplates.find(t=>Number(t.id)===Number(ch.taskTemplateId)); if(!task) return 0; return state.entries.filter(e=>e.createdAt?.slice?.(0,10)===state.challengeDate&&e.title===task.name).length;};
const applyChallengeBonuses=state=>{state.dailyChallenges.forEach(ch=>{const progress=countProgress(state,ch); const key=`${state.challengeDate}_${ch.id}`; if(progress>=ch.target&&!state.challengeBonusClaims[key]&&ch.bonus>0){state.challengeBonusClaims[key]=true; state.players.forEach(p=>{p.steps+=ch.bonus; state.entries.push({id:crypto.randomUUID(),playerName:p.name,title:'Team-Bonus',points:ch.bonus,category:'Challenge-Bonus',note:'Tages-Challenge erfüllt',manual:true,createdBy:'system',createdAt:new Date().toISOString()});});}});};
module.exports=async(req,res)=>{
 if(req.method!=='POST') return json(res,405,{error:'Method not allowed'});
 const session=getSession(req); if(!session) return json(res,401,{error:'Nicht eingeloggt'});
 try{
  ...
  if(action==='renamePlayer'){
    if(!isAdmin(session)) return json(res,403,{error:'Nur Admin'});
    const oldName=String(payload.oldName||''), newName=String(payload.newName||'').trim();
  ...
  if(action==='resetPassword'){
    if(!isAdmin(session)) return json(res,403,{error:'Nur Admin'});
    const {error}=await supabase.from('app_users').update({password_hash:String(payload.newPassword||''),updated_at:new Date().toISOString()}).eq('username',String(payload.username||'')); if(error) throw error;
    return json(res,200,{ok:true});
  }
  if(action==='changeOwnPassword'){
    const {error}=await supabase.from('app_users').update({password_hash:String(payload.newPassword||''),updated_at:new Date().toISOString()}).eq('username',session.username); if(error) throw error;
    return json(res,200,{ok:true});
  }
